...
//        .collect(Collectors.toMap(Employee::getDepartment, Employee::getName));
                ...
                .collect(
                        Collectors.groupingBy(Employee::getDepartment,
                                Collectors.collectingAndThen(
                                        Collectors.maxBy(Comparator.comparingDouble(e -> e.getSalary().doubleValue())),
                                        e -> e.orElse(null)
                                )));
    }
    ...
    public Double calculateAverageMarks(String[][] studentsMark, Comparator<? super Entry<String, Double>> comparator) {
       ...
       /*
       Arrays.stream(studentsMark)
        .map(e -> Map.entry(e[0], Integer.parseInt(e[1])))
        .collect(Collectors.groupingBy(Entry::getKey, Collectors.averagingDouble(Entry::getValue)))
        .values()
        .stream()
        .mapToDouble(Math::floor)
        .max()
        .orElse(0.0);
        */
    }

    public Map<String, Long> caseInSensitiveWordFrequency(String statement) {
        String[] words = statement.toLowerCase().split(" ");
        return Arrays.stream(words)
                .collect(Collectors.groupingBy(Function.identity(), Collectors.counting()));
    }

    public Map<String, Object> flattenObject(Map<String, Object> input) {
    ...
    private static void flattenHelper(Map<String, Object> current,
        ...
        for (Map.Entry<String, Object> entry : current.entrySet()) {
            ...
            if (value instanceof Map) {
                flattenHelper((Map<String, Object>) value, newKey, result);
            } else {
    ...
    public List<Map<String, Object>> merge(List<Map<String, Object>> input) {
        ...
        for (Map<String, Object> obj : input) {
            ...
            if (!grouped.containsKey(name)) {
                grouped.put(name, new LinkedHashMap<>(obj));
            } else {
                Map<String, Object> existing = grouped.get(name);
    ...
    private void deepMerge(Map<String, Object> target, Map<String, Object> source) {
        for (String key : source.keySet()) {
            ...
            if (value instanceof Map && target.get(key) instanceof Map) {
                deepMerge((Map<String, Object>) target.get(key),
    ...
    public List<Map<String, Object>> mergeArrays(List<Map<String, Object>> first, List<Map<String, Object>> second) {
        List<Map<String, Object>> result = new ArrayList<>();
        // Assume first and second has same length
        for(int i = 0; i < first.size(); i++) {
            Map<String, Object> mapOne =  first.get(i);
            Map<String, Object> mapTwo = second.get(i);
    ...
    public Map<String, List<String>> mergeByValue(Map<String, String> employees) {
