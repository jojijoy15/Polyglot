...
public class MinimumPathSum {
    ...
    /*
    You are given an m × n grid filled with non-negative integers.
    ...
    Your task is to find the minimum possible sum of all numbers along a path from the start to the destination.

    Example
        Input: [ [1, 3, 1], [1, 5, 1], [4, 2, 1] ] Output: 7

        Explanation:
        One optimal path is: 1 → 3 → 1 → 1 → 1
        Sum = 7
    */
    ...
    public int minSumOptimized(int[][] grid) {
        int rows = grid.length;
        int cols = grid[0].length;
